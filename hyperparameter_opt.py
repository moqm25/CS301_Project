...
### Search Space [28 x 6]###
learning_rate_space = np.linspace(0.01, 2.5 * (10 ** -5)) #[]
# for e in [-2, -3, -4]:
...
def normal_dist(x , in_arr):
    prob_density = (np.pi*in_arr[1]) * np.exp(-0.5*((x-in_arr[0])/in_arr[1])**2)
    return prob_density
...
def likelihood(newValue, existingSet):
    distances = [min(np.abs(v - np.array([x for x in existingSet if x != v]))) for v in existingSet]
    input_arr = np.array([existingSet, distances]).T
    return np.sum([normal_dist(newValue, ia) for ia in input_arr], axis=0)
...
def find_best_params():
    ...
    try:
        ...
        return -1, -1
    return learning_rate_space[lr_evals[-1]], filter_count_space[fcf_evals[-1]]
