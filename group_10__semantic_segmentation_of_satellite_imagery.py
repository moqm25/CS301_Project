...
if os.path.isdir("models") and os.path.isfile("models/satellite_standard_unet_20epochs.hdf5"):
    model = load_model("models/satellite_standard_unet_20epochs.hdf5",
                   custom_objects={'dice_loss_plus_1focal_loss': total_loss,
                                   'jacard_coef':jacard_coef})
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=10**-6), loss=total_loss, metrics=metrics)
else:
    model = get_model()
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=10**-6), loss=total_loss, metrics=metrics)
...
history1 = model.fit(X_train, y_train, 
                    ...
                    shuffle=True)
model.save("models/satellite_standard_unet_20epochs.hdf5")
